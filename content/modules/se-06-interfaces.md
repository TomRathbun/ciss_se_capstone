...
| Content    | What data / commands are exchanged?               | Excel workbook with time‑keeping sheet, discrepancy tracker |
...
| Errors     | What if missing / late / invalid?                 | Export fails → error code 502, email notification to manager |
| Security   | Who is allowed?                                   | Authenticated ETAS service only; file encrypted at rest |
...
graph LR
    ...
    style Cons fill:#fff3e0,stroke:#ef6c00,stroke-width:2px
```
...
>"Reminder: If any wording in the ICD is generated with AI, add a citation (e.g., Generated with ChatGPT, 2026). The underlying design decisions must be yours.


#### f. Workshop prompt with timing and sharing
...
**V&V and traceability** – you will verify that each external interface meets the success criteria you defined here and that the traceability matrix connects every requirement to a concrete test.
